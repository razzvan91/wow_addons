...
local DoSelectFilter=function(state)
	for index=1,GBB.MAXDUNGEON-2 do ---trade -misc
		ChkBox_FilterDungeon[index]:SetChecked(state)
	end	
end
...
function GBB.OptionsInit ()
	...
	for index=1,GBB.MAXDUNGEON-2 do -- -trade -misc
		ChkBox_FilterDungeon[index]:OnRightClick(DoRightClick)
	end	
